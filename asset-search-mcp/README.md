...
- **`roblox_search_assets`** — ranked, multi-category Toolbox v2 search with vote/creator/
  script/mesh metadata and ids. `extensive=true` expands the query into variants
  for broader exploration.
- **`roblox_plan_game_asset_coverage`** — turns a Roblox game idea into search slots for
  lobby spawn, NPCs, portals, upgrade/cosmetic surfaces, and themed room packs.
- **`roblox_preprocess_storyboard_asset_cache`** — turns coverage slots into a bounded
  cache-warming/storyboard input plan, optionally running searches before any
  Studio work.
- **`roblox_export_asset_brain_snapshot`** — emits a compact GitHub Pages-friendly
...
- **`roblox_curate_assets`** — a storyboard's slots → a diversity-capped shortlist per
  slot (no single creator's pack dominating).
- **`roblox_review_asset` / `roblox_get_reviews`** — persist an agent's keep/reject verdict so
  other agents reuse it instead of re-vetting.
- **`roblox_record_inspection` / `roblox_record_inspections` / `roblox_get_inspection`** — persist StudioMCP-measured
  geometry, safety facts, and player-angle visual risk without coupling this
  server to Studio.
- **`roblox_record_asset_permission` / `roblox_validate_publish_permissions`** — persist
...
- **`roblox_commit_palette` / `roblox_get_palette`** — freeze the chosen asset per slot for the
  build phase, optionally refusing assets without publish-permission proof.
- **`roblox_validate_prop_hunt_gate`** — validate the committed Prop Hunt asset palette
  before a live Studio build/playtest.
- **`roblox_plan_headless_assembly` / `roblox_validate_fragment_manifest`** — define and check
  referent-safe `.rbxm` fragment packets for parallel headless place assembly.
- **`roblox_plan_playable_space_review` / `roblox_validate_playable_space_review`** — plan
...
- **`roblox_plan_world_asset_family_sweep` / `roblox_validate_world_asset_family_sweep`** —
...
- **`roblox_plan_batch_visual_gate` / `roblox_validate_batch_visual_gate`** — wrap a
...
- **`roblox_plan_ai_game_dev_loop` / `roblox_validate_ai_game_dev_loop`** — top-level custom
...
State persists in **SQLite** (`~/.roblox-asset-brain/asset-brain.sqlite`) via a
DAL/DAO layer (`src/dal/`) using Node's built-in `node:sqlite` — still no native
deps, no build step, but now **cross-process safe** (WAL + transactions): parallel
agent sessions share one brain without clobbering each other, and claims are
race-free. Requires **Node >= 22.5**. Legacy `*.json` brain files are imported
automatically once on first run. The brain stores metadata only: IDs, reviews,
inspections, visual verdicts, permission proof, and palette choices.

All tools are prefixed `roblox_` so they can't collide with other Roblox MCP
servers (the Studio MCP exposes a bare `search_assets`). API/network outages
surface as explicit tool errors and are never cached — an empty result always
means the Creator Store really returned nothing.
...
```bash
claude mcp add --transport stdio roblox-asset-search -- node "$(pwd)/src/index.js"
claude mcp list     # confirm roblox-asset-search + StudioMCP both connected
```
...
| Tool | Purpose |
|------|---------|
| `roblox_plan_ai_game_dev_loop(project?, game?, target_place?, themes?, include_defaults?, include_lobby?, max_themes?, max_fragments?, assembly_profile?, review_mode?, spaces?, include_default_spaces?, artifact_root?, max_captures?, format?)` | Plan the full AI Roblox game-dev loop across asset brain, GameKit, parser/writer generation, headless merge, gated Studio batch proof, and release verification. |
| `roblox_validate_ai_game_dev_loop(report, plan?, format?)` | Validate the final proof report for the full AI game-dev loop, including custom MCP contract proof and the nested batch visual gate report. |
| `roblox_plan_project_template(project?, game?, target_place?, themes?, output_root?, format?)` | Plan a new Roblox AI game skeleton with asset brain, prompt lanes, Rojo stubs, POC script, and gates prewired. |
| `roblox_validate_project_template(report, plan?, format?)` | Validate a generated project template report and required files. |
| `roblox_plan_asset_acquisition(project?, slot?, query?, asset_ids?, target_place?, delivery_mode?, require_publish_permission?, format?)` | Plan the asset acquisition seam: search/claim, publish permission proof, direct asset delivery parse, Studio insertion fallback, quarantine, manifest validation, and visual proof. |
| `roblox_validate_asset_acquisition(report, plan?, format?)` | Validate an acquisition proof report before an asset can move from quarantine into a palette. |
| `roblox_plan_asset_delivery(project?, slot?, asset_id, version_number?, quarantine_root?, base_url?, api_key_env?, bearer_env?, format?)` | Plan one authenticated Open Cloud Asset Delivery request that writes bytes to quarantine and records only a redacted receipt. |
| `roblox_validate_asset_delivery_receipt(receipt, request?, format?)` | Validate an Asset Delivery receipt: redacted auth proof, 2xx response, non-empty bytes, sha256 digest, and no asset-brain binary paths. |
| `roblox_plan_game_asset_coverage(game?, themes?, include_defaults?, include_lobby?, max_themes?, format?)` | Generate lobby/session/room asset search coverage before curation. |
| `roblox_preprocess_storyboard_asset_cache(project?, game?, themes?, include_defaults?, include_lobby?, max_themes?, max_slots?, warm_search_cache?, per_slot?, verified_only?, extensive?, format?)` | Build storyboard-ready coverage slots, optionally warm ranked search cache, and return Pages-friendly metadata layout. |
| `roblox_export_asset_brain_snapshot(project?, include_search_cache?, max_queries?, max_results_per_query?, format?)` | Export small metadata-only snapshot for GitHub Pages or handoff; no binaries/screenshots. |
| `roblox_plan_headless_assembly(project?, target_place?, themes?, include_lobby?, max_fragments?, format?)` | Generate parallel headless fragment packets, merge contract, endpoints, Rojo/Lune validation commands, and Studio visual gate. |
| `roblox_validate_fragment_manifest(manifest, format?)` | Reject unsafe or under-specified `.rbxm` fragment manifests before coordinator merge. |
| `roblox_plan_coordinator_merge(adapter?, place?, out?, fragments?, replace_existing?, create_missing_targets?, report_path?, lune_command?, rbx_dom_command?, rbx_dom_args?, format?)` | Plan a replaceable headless merge through the proven Lune adapter or an external rbx-dom coordinator command. |
| `roblox_validate_coordinator_merge(report, plan?, format?)` | Validate a coordinator merge report: process passed, output reload validated, fragments match plan, identity policy is coordinator-owned, and outputs avoid asset-brain. |
| `roblox_plan_playable_space_review(project?, review_mode?, spaces?, include_defaults?, format?)` | Generate the required Studio screenshot queue for lobby/room quadrant review and UI states. `review_mode="player_angle"` emits only player-height quadrant shots for scoped asset-fix passes. |
...
| `roblox_plan_world_asset_family_sweep(project?, target_place?, families?, artifact_root?, max_families?, format?)` | Plan a one-family-at-a-time Studio verification pass for sideways, face-down, floating, buried, mis-scaled, or inconsistently placed assets. Requires clean-clone before/after views, live player-height proof, canonical orientation/scale/grounding metadata, propagation to all live instances, and temp cleanup. |
| `roblox_validate_world_asset_family_sweep(report, plan?, format?)` | Fail reports that fix only a temporary clone, skip required clean/live screenshots, omit canonical placement metadata, miss `roblox_record_inspection` proof, leave blockers, or leave temporary validation models behind. |
| `roblox_plan_batch_visual_gate(project?, target_place?, review_mode?, spaces?, include_defaults?, adapter?, artifact_root?, max_captures?, format?)` | Turn a playable-space review plan into one serial StudioMCP wrapper packet with active-place preflight, camera Luau, `screen_capture` requests, collation paths, and a report template. |
| `roblox_validate_batch_visual_gate(batch_report, plan?, format?)` | Validate a collated Studio screenshot batch: active-place proof must pass, every planned capture needs an image path, and the embedded playable-space report must pass. |
| `roblox_search_assets(query, max_results?, categories?, verified_only?, extensive?, exclude_terms?, exclude_rejected?, exclude_claimed?, exclude_ids?, exclude_unpublishable?, publish_permission_mode?, require_studio_probe?, require_save_reopen?)` | Ranked search; auto-hides rejected/claimed; `exclude_terms` drops off-theme names; optional release-mode permission filtering uses recorded proof. |
| `roblox_curate_assets(slots[], per_slot?, verified_only?, extensive?, exclude_terms?, exclude_claimed?, exclude_unpublishable?, publish_permission_mode?, require_studio_probe?, require_save_reopen?)` | Diverse shortlist per slot; excludes rejected + claimed; no asset suggested for two slots; optional permission filtering for release palette curation. |
| `roblox_claim_assets(project, slot, asset_ids[], reviewer?)` | Reserve assets to a slot so other agents' results hide them — prevents collisions. |
| `roblox_reject_asset(asset_id, reason, slot?, reviewer?)` | Shared veto: the asset is auto-excluded from every agent's future results. |
| `roblox_review_asset(asset_id, verdict, slot?, rating?, notes?, reviewer?)` | Persist a shared verdict (`reject` auto-excludes). |
| `roblox_get_reviews(asset_id)` | Read all verdicts + claim status for an asset. |
| `roblox_record_inspection(asset_id, slot?, size_studs?, has_scripts?, script_count?, base_part_count?, anchored_capable?, primary_part?, issues?, visual_risks?, visual_risk_score?, screenshot_verdict?, reviewer?, source?)` | Store the latest StudioMCP-measured and player-angle visual facts for an asset. |
| `roblox_record_inspections(inspections[])` | Store many StudioMCP inspection records in one call after a live palette audit. |
| `roblox_get_inspection(asset_id)` | Read the latest persisted Studio inspection for an asset. |
| `roblox_record_asset_permission(asset_id, target_publisher?, target_experience_id?, access, grantable_by_us?, experience_has_access?, publish_policy?, studio_insert_probe?, save_reopen_probe?, dependencies?, evidence?, notes?, reviewer?, source?)` | Store permission proof for one asset and its dependencies. |
| `roblox_record_asset_permissions(permissions[])` | Store many permission records from a dashboard export or Studio audit. |
| `roblox_get_asset_permission(asset_id, publish_permission_mode?, require_studio_probe?, require_save_reopen?)` | Read permission proof plus evaluated release readiness. |
| `roblox_validate_publish_permissions(project?, publish_permission_mode?, require_studio_probe?, require_save_reopen?, format?)` | Fail a palette when any committed asset lacks publish permission proof. |
| `roblox_release_claim(asset_id)` | Release one claim so other agents can pick the asset again. |
| `roblox_release_stale_claims(max_age_hours?, dry_run?)` | Release claims older than the cutoff (default 168h) left behind by crashed/abandoned agent sessions. |
| `roblox_brain_status()` | Brain health: backend, dir, cache freshness, and counts of reviews/claims/inspections/permissions/palettes. |
| `roblox_prune_search_cache(ttl_hours?, dry_run?)` | Delete expired search-cache rows (also runs automatically at startup). |
| `roblox_commit_palette(project, slot, asset_id, name?, require_publish_permission?, publish_permission_mode?, require_studio_probe?, require_save_reopen?)` | Freeze a chosen asset per slot (also claims it); strict mode refuses assets without publish proof. |
| `roblox_get_palette(project)` | Read the committed palette for the build phase, including publish-readiness status. |
| `roblox_validate_prop_hunt_gate(project?, min_areas?, min_hideable_total?, min_setpiece_total?, min_hideable_per_area?, min_setpiece_per_area?, min_hideable_studs?, max_hideable_studs?, require_inspections?, require_primary_part?, format?)` | Check the committed Prop Hunt palette before Studio build. |
...
Parallel agents share one persistent store. The loop: `roblox_curate_assets` (already
filtered) → `roblox_claim_assets` the shortlist (peers now skip it) → inspect in Studio
→ `roblox_reject_asset` the duds (shared veto) → `roblox_commit_palette` the winner. Because
...
`roblox_record_inspection`. After screenshot review, include `visual_risks`,
...
`roblox_search_assets` and `roblox_curate_assets` also accept `exclude_unpublishable=true` for
...
Use `roblox_plan_game_asset_coverage` before curation whenever the skill is building or
...
Feed the returned slots into `roblox_curate_assets(..., extensive=true)`, claim the
shortlists, inspect in Studio, then commit the best assets to the palette.

For cache-first storyboarding, call `roblox_preprocess_storyboard_asset_cache` first.
...
Use `roblox_export_asset_brain_snapshot(format="json")` to mirror the current asset
...
Use `roblox_plan_asset_delivery` or the CLI to fetch candidate model bytes before
...
stores a redacted delivery receipt. Run `roblox_validate_asset_delivery_receipt` before
including that receipt in `roblox_validate_asset_acquisition`. Keep the receipt path
metadata in the asset brain; keep downloaded `.rbxm` bytes out of it.

## Project template generation

Use `roblox_plan_project_template` or the CLI to start a new game repo skeleton with
...
Use `roblox_plan_headless_assembly` when multiple agents should build room/lobby
...
Every agent fragment must pass `roblox_validate_fragment_manifest` before merge. The
...
Use `roblox_plan_playable_space_review` after building or changing a map. The default
...
```text
roblox_plan_playable_space_review(project: "eggbreakers", review_mode: "player_angle", spaces: [...])
```
...
After screenshot review and fixes, call `roblox_validate_playable_space_review` with a
...
Use `roblox_plan_world_asset_family_sweep` before full playable-space signoff when
...
For lower-churn validation, use `roblox_plan_batch_visual_gate`. It emits one wrapper
...
collation. Feed the wrapper output to `roblox_validate_batch_visual_gate`; it fails if
...
`roblox_validate_prop_hunt_gate(project: "prophunt")`. Defaults match the current
...
src/store.js     service layer: TTL cache, single-flight, reviews, claims, inspections, palette
src/dal/         SQLite DAL: database.js (connection/schema/legacy import), daos.js (one DAO per table)
